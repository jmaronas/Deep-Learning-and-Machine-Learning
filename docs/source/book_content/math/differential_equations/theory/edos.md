...
Without loss of generality, beyond changing exponentials to matrix exponentials, we will be assuming vector-valued functions, i.e,  we could either have $y: \mathbb{R} \rightarrow \mathbb{R^d}$ or $y: \mathbb{R^n} \rightarrow \mathbb{R^d}$, although for exposition we will always be thinking of functions from the time domain. I like to make time dependence explicit.

## Homogeneous

A homogeneous ODE is given by:
...
A non-homogeneous ODE is given by:
...
Note that here the solution has an integral given by the convolution between two signals, and this is the reason why Linear Time Invariant systems use convolution to obtain the output, something I was taught as: "this is like this because I say it" rather than giving out the real reasons. Obviously, I need to elaborate further on this claim to show where this really comes true for any convolution.

## Homogeneous Time Variant ODE

A homogeneous time-variant ODE is given by:
...
where the function $\Psi$ must satisfy some properties; see the book by Arno Solin, equation 2.34.
...
$$
y(t) = \Psi(t,t_0)y(t_0) + \int^t_{t_0} \Psi(t,\tau) L(\tau) w(\tau)d\tau;
$$
...
First, note that a non-homogeneous ODE generalizes a homogeneous ODE. Basically, the solution is the homogeneous one plus the fluctuations introduced by the other function through a convolution. So, in other words, if we set $L=0$  and $C=0$, we recover the homogeneous solution.
